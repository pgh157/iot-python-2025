...
class Block:
    ...
    def move(self): #볼 무브
        # 볼이 움직이는 x축 값을 계속 계산하려면 x은 dir 값을 라디언으로 변환후 코사인처리
        self.rect.centerx += math.cos(math.radians(self.dir))*self.speed
        # 볼이 움직이는 y축 값을 계속 계산하려면 y은 dir 값을 라디언으로 변환후 사인처리
...
def main():
    ...
    for y, color in enumerate(colors, start=0): # y값은 0 ~ 6
        for x in range(0,9):
            BLOCK.append(BLOCK(colors, Rect(x*80 + 150, y*40 + 40, 60 , 20)))

    bigFont = pygame.font.SysFont("NanunmGothic",80)
    ...
    while True:
        ...
        if is_game_start == False:
            ...
            elif BALL.rect.centery < 0 : ## 게임화면 천장에 부딫히면 반사
                BALL.dir=-BALL.dir
