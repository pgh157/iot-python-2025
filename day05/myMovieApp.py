...
VERSION = 0.5  #$ 변수를 상수로 사용하고 싶으면 대문자로...
...
def load_movie(items: list):
    f = open("movie_db.txt", encoding="utf-8", mode="r") # 파일쓰기로 오픈
    while True:
...
def set_movie():
    title, year, company, rate = input("영화입력[제목|개봉연도|제작사|평점] > ").split("|") # 입력 중 발생하는 예외
    year = int(year) # 년도는 정수로
    rate = float(rate)
    # print(title, year, company, rate)
    # movie = Movie(title, year, company, rate) #$ 밑에 꺼랑 구문 같음
